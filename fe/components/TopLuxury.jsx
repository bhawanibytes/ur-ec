...
export default function TopLuxury() {
  const { getLuxuryProperties, loading, error } = useProperties();
  const properties = getLuxuryProperties(10);
    ...
    <section className="p-md-4 p-2 py-md-5 bg-light my-5">
      <div className="container px-3 px-md-0">
        <div className="d-flex justify-content-between align-items-center mb-4 mb-md-5">
          <div>
            <h2 className="display-6 fw-bold text-dark mb-2">TOP LUXURY PROPERTIES</h2>
            <p className="text-muted mb-0">Exclusive properties above ₹10 Cr</p>
          </div>
          <Link 
            href="/properties" 
        ...
        {loading ? (
          ...
          <div className="text-center py-5">
            <p className="text-muted">No luxury properties available at the moment.</p>
          </div>
        )}
      </div>
    </section>
